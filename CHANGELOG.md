...
## 2026-09-08 (corrige nome próprio errado no ditado por voz)

- Usuário testou o ditado por voz (funcionando, depois de um refresh forçado — o navegador estava com JS antigo em cache) e perguntou se dava pra melhorar a precisão da transcrição. A API de reconhecimento do navegador não tem parâmetro de vocabulário (isso só existe em serviço pago tipo Google Cloud Speech) — o único ponto de entrada disponível é corrigir DEPOIS que o texto já saiu. Duas formas: dicionário local (grátis) ou mandar o texto pra Groq revisar (mais inteligente, mas gasta a mesma cota diária que já estourou várias vezes hoje). Escolhido o dicionário local.
- **`src/lib/voice-correction.ts`**: compara cada trecho do texto ditado contra os nomes de cliente da carteira, nomes de quem está no squad, e um pequeno glossário do playbook (Oxy, Kickoff, Onboarding, Fechamento, Comitê Estratégico Mensal, Diagnóstico, Balancete...) — trocando pela grafia certa quando a distância de edição for pequena o bastante. Não mexe em gramática nem pontuação, só nome próprio, que é a categoria de erro que mais atrapalha (o resto o próprio usuário revisa antes de mandar).
- **Limiar por distância de edição absoluta, não por porcentagem** — foi preciso ajustar depois do primeiro teste: um limiar tipo "72% parecido" praticamente nunca deixa passar termo curto ("oxi" pra "Oxy" já é 1 edição em 3 letras, ~67% de semelhança, abaixo de qualquer limiar percentual razoável, mesmo sendo o erro mais óbvio de todos). Banda por tamanho do termo (≤4 letras tolera 1 edição, ≤8 tolera 2, mais que isso tolera 3) resolve isso sem abrir a porta pra corrigir palavra comum por engano.
- **Correção sem acento também precisou de ajuste**: "zé do flor" ditado sem o acento tem distância ZERO contra "Zé do Flor" depois de normalizar os dois lados — o código original via isso como "já está certo" e não corrigia a grafia. Comparação passou a checar o texto ORIGINAL (com acento/maiúscula) contra o termo antes de decidir se "já está certo", só usando a versão normalizada pra medir a distância de edição.
- **Pontuação colada na palavra não pode sumir**: "feliphe," virando "Felipe" sem a vírgula era efeito colateral de substituir o token inteiro. Corrigido preservando o que vier antes/depois da palavra (vírgula, ponto) ao redor da substituição.
- Vocabulário buscado uma vez quando o painel abre (mesmo efeito que já carrega o histórico da conversa, sem somar mais um achado de lint) — nomes de cliente vêm de `/api/clients`, nomes do squad de `/api/users`, ambos já cacheados pelo navegador. Só busca se o navegador tiver a API de voz — sem isso, seria rede gasta à toa.
- Validado: typecheck e lint limpos (sem achado novo), `next build` compilando. **15 asserções contra a função de verdade** (não uma cópia) via rota temporária no dev server, sem precisar de microfone nem de IA: nome de cliente e pessoa mal reconhecidos, nome de cliente com várias palavras, termo do playbook, texto já correto não muda, palavra comum não vira nome por engano, nome repetido duas vezes corrige as duas, só capitalização errada corrige, frase longa realista, e pontuação preservada. Rota de teste apagada antes do commit.
...
- **Escopo trocado de `calendar.readonly` pra `calendar.events`** (leitura+escrita de evento, sem mexer em agenda/configuração) — cobre o que o sync já fazia e adiciona criar. **Só vale pra quem reconectar depois desta mudança**: contas já vinculadas mantêm o escopo antigo até passar pelo login do Google de novo (mesmo motivo do `gmail.compose` ainda não valer pra ninguém).
- **A ação mais sensível de todo o assistente, tratada como tal**: `agendar_evento` cria um compromisso de verdade na agenda do Google de quem confirmar — não é o Kanban interno, é a agenda real da pessoa. Por isso, diferente de `criar_tarefa`/`criar_cliente` (que executam direto), esta é **sempre confirmação, sem exceção**, e **nunca aceita convidado** nesta primeira versão — sem isso, um pedido mal-entendido viraria e-mail de convite de verdade pro cliente. `sendUpdates: "none"` na chamada da API garante que não dispara notificação nenhuma.
- Novo tipo de alvo `calendar_new` no `AssistantAction` (o mesmo mecanismo genérico de tarefa/tratativa/cliente, sem precisar mexer na rota de confirmação nem no card do chat — ambos já eram agnósticos ao tipo). Ao confirmar, `createCalendarEvent` (`src/lib/calendar-write.ts`) usa o mesmo padrão de autenticação do sync (token OAuth da própria pessoa, sem service account) e **espelha o evento na hora** em `CalendarEvent` — sem isso a reunião só apareceria em `/calendar` no próximo sync do cron, e a pessoa acabou de pedir "agora".
- **Erro de permissão vira instrução, não trava crua.** Conta sem o escopo novo faz a chamada à API do Google falhar com 403/insufficient scope — em vez de estourar isso pro chat, a função detecta esse padrão e devolve "precisa reconectar em Configurações", e o prompt instrui o assistente a não tentar de novo sozinho depois desse erro.
- Validado: typecheck, lint (o único achado é pré-existente em `auth.ts`, fora da linha que toquei) e `next build` limpos. **Não dava pra testar contra o Google de verdade** — não tenho conta de teste, e criar um evento real seria inapropriado só pra validar código. Testado o que dava sem tocar a API do Google: 5 asserções confirmando que confirmar sem conta conectada devolve erro claro (não 500), não cria `CalendarEvent` nenhum, e deixa a ação `pending` (dá pra tentar de novo depois de reconectar) em vez de marcar como resolvida com erro; mais 9 de 10 (a 10ª "falha" era aritmética errada do meu próprio script de teste, confirmada lendo as linhas direto do banco) validando a validação de data/horário/duração e que nada é criado antes da confirmação.
- **Pendência que passa a ser sua**: pra isso funcionar de verdade, alguém do squad precisa reconectar o Google (Configurações) — vale avisar o squad pra fazer isso de uma vez só, já que cobre o rascunho de cobrança pendente desde agosto e o calendário novo.

## 2026-09-08 (mais 7 ações no assistente: link, checklist, lembrete, tratativa e cliente)

- Usuário pediu mais ações depois do primeiro lote (criar/alterar tarefa) ter funcionado. Levantado o que já tinha rota pronta (`/api/tasks/[id]/links`, `/api/tasks/[id]/remind`) contra o que era território novo (Tratativas e ClientNote, que o assistente não enxergava nem mexia). Escolhas do usuário: as 3 de baixo risco direto (link, checklist, lembrete) mais os 4 territórios novos — tratativa nova, tratativa existente, cliente novo, cliente existente.
- **Executam direto** (mesmo critério de sempre — errar é barato): `anexar_link_tarefa`, `marcar_item_checklist` (marca/desmarca item que já existe; pra criar um novo continua sendo `adicionar_item_checklist`), `enviar_lembrete` (mesmo Slack do botão "Lembrar" da tela) e `criar_cliente`.
- **Pedem confirmação** (mexem em registro que o squad usa pra reportar número — funil de churn, dashboard de clientes): `registrar_tratativa`, `alterar_tratativa` e `editar_cliente`.
- **`AssistantAction` generalizado** de "só tarefa" pra três tipos de alvo (`task` | `tratativa` | `tratativa_new` | `client`), trocando o campo `taskId` por `targetType`/`targetId` — reformado sem migração de dados porque a tabela estava zerada (0 linhas). A rota de confirmação (`POST /api/assistant/actions/[id]`) não mudou nada por fora: o botão no chat já é genérico (mostra o `summary`, chama confirmar/cancelar), então herdou os três tipos novos de graça.
- **Extraído `src/lib/client-resolve.ts`** com `resolveClientName` e `knownClientNames`, que viviam duplicados em `assistant-tools.ts` e em `/api/clients/route.ts` — um lugar só evita a regra de "achar cliente por nome parecido" divergir entre o assistente e a tela.
- **Dois bugs achados pelo teste, com causas bem diferentes:**
  - **`list_clients` derrubava a conversa inteira.** A própria Groq rejeitava a chamada (400, antes de chegar no nosso código) porque o modelo mandou `healthStatus: null` em vez de omitir a chave — hábito comum dele em parâmetro opcional que não usou. Corrigido em dois níveis: os 13 campos opcionais de enum nos schemas passaram a aceitar `null` (pra Groq parar de rejeitar a chamada), e `runTool` agora limpa `null` pra `undefined` antes de despachar (pra "isso foi pedido" continuar significando o que sempre significou nas funções de cada ferramenta — sem isso, `status: null` seria lido como "mude o status pra null" em vez de "não mexeu nisso"). Como o padrão `type: "string", enum: [...]` se repetia em 14 lugares, a mesma causa provavelmente já afetava ferramentas antigas silenciosamente.
  - **Item de checklist já marcado, pedido pra marcar de novo sem `done` explícito no argumento → desmarcou.** O design original alternava o valor quando `done` vinha ausente; provou ser perigoso assim que o modelo, no teste, chamou a ferramenta sem passar o parâmetro — o toggle inverteu um item que já estava certo, o oposto exato do pedido. Trocado: ausente (ou `true`) marca, que é o caso comum; só desmarca com `done: false` explícito. Sem alternância nenhuma.
- **Tratativa ambígua ganhou desambiguação por texto.** `resolveTratativa` aceita agora uma dica de motivo opcional (`alterar_tratativa.motivo`) — quando o cliente tem mais de uma tratativa aberta, ela primeiro tenta estreitar pelo trecho do motivo antes de listar tudo e desistir. Sem isso, a segunda pergunta ("a que é sobre X") não tinha como ser resolvida — o assistente ia devolver a mesma lista de novo, em loop.
- Validado: typecheck e lint limpos, `next build` compilando (dependências conferidas de novo depois do susto de ontem à noite — `next` 16.2.9 / `prisma` 7.8.0, intactas). **Duas rodadas de teste real com a Groq** (17 e depois 18 asserções, cobrindo os 7 caminhos novos) mais **8 asserções determinísticas** — feitas chamando `runTool` direto por uma rota temporária, sem gastar cota nenhuma da IA — que provam as duas correções isoladas do resto: toggle de checklist não alterna mais, e `status: null`/cliente inexistente não quebram `editar_cliente`. A rota temporária foi apagada antes do commit.
- **Duas pegadinhas do próprio `.next`/Turbopack achadas no meio do teste**, reforçando a nota que ficou registrada hoje mais cedo: rota nova criada com o dev server já rodando dá 404 até reiniciar (aconteceu de novo com a rota de teste), e pasta com `_` no início (`_test_tool`) é convenção de "privada" no App Router — nunca vira rota, custou uma rodada extra de debug até eu perceber e renomear.
- **Honestidade sobre a cobertura:** a cota diária da Groq estourou duas vezes no meio dos testes (é cota compartilhada com a extração de Meet Recaps), então nem toda asserção veio de uma única rodada limpa com IA — as duas que a cota impediu de confirmar via linguagem natural (cliente inexistente em `editar_cliente`, comportamento do checklist) foram cobertas pela rota determinística.
- Squad sintético removido ao final. Conferido no banco: 257 tarefas, 10 clientes, 5 squads reais, 0 `AssistantAction` sobrando. **Achado que vale registrar, não é bug meu:** o banco tem **0 tratativas** em qualquer squad, incluindo o da O2 — rastreei os deletes de todos os scripts de teste e todos eram escopados por `squadId` do squad sintético, então não foi limpeza que apagou; parece ser o estado real (funcionalidade sem uso ainda).

## 2026-09-08 (assistente de IA passa a agir: cria e altera tarefa, sem apagar nada)

- Usuário perguntou se dava pra deixar o assistente executar ações de verdade, não só consultar. Escolhas dele: **criar e alterar tarefa, nunca apagar**, e alteração em tarefa existente **só depois de clicar em Confirmar** no chat. A linha foi traçada por **custo do erro**, não por tipo de ação.
- **Executa direto** (`criar_tarefa`, `comentar_tarefa`, `adicionar_item_checklist`): errar ao criar é barato — nasce visível no Kanban e some com um clique. `criar_tarefa` cria Task de verdade (diferente de `propor_tarefa`, que segue existindo pro caso de pedido vago ou ideia da própria IA), resolve responsável pelo nome e aceita 2+ nomes virando tarefa em conjunto.
- **Pede confirmação** (`alterar_tarefa`): status, prazo, horário, prioridade, responsável e cliente. A ferramenta **não altera nada** — grava um `AssistantAction` pendente e devolve o resumo; quem muda o dado é `POST /api/assistant/actions/[id]`, chamado pelo botão. O motivo dessa camada é específico: **o erro provável do assistente não é escolher a ação errada, é errar o ALVO** ("muda o prazo da tarefa do balancete" com três balancetes abertos). Por isso `resolveTarget()` se recusa a desempatar — com mais de um candidato devolve a lista pro assistente perguntar qual, em vez de chutar. Confirmado no teste: com duas tarefas de balancete ele respondeu "Qual delas?" e não preparou ação nenhuma.
- **Não existe ferramenta de apagar**, de propósito: excluir cliente faz cascade em 7 tabelas e excluir tarefa leva comentários e histórico junto. Continua sendo decisão de tela, com os dois níveis de confirmação que já existem lá. Pedido pra apagar no chat recebe explicação, não ação.
- **Teto de alcance**: uma ação = uma tarefa. Não existe caminho pra `updateMany` — se pedirem mudança em lote, o assistente trata uma de cada vez. É o que separa um erro de um estrago.
- **Rastro**: toda alteração entra no histórico da tarefa como *"Fulano (via assistente)"* e no `PlatformLog`; comentário criado pela IA sai assinado. Dá pra auditar depois o que foi mão humana e o que não foi.
- **Guardas da rota de confirmação** (é ela que muda dado, então concentra a proteção): a `AssistantAction` carrega o `userId` de quem pediu e é conferido contra a sessão — ninguém confirma ação preparada por outra pessoa nem de outro squad; ação já resolvida não confirma de novo; expira em 30 min; "Nova conversa" cancela a pendente. Concluir tarefa em conjunto por aqui marca todas as partes, igual à tela.
- Pra o botão existir, `ToolContext` ganhou um canal de volta (`pendingActionId`) que a rota do chat lê e devolve como `pendingAction` junto da resposta; `GET /api/assistant/messages` também devolve a pendente, pra o botão não sumir se a pessoa der F5 no meio.
- **Dependências mexeram sozinhas no meio do trabalho** e travaram tudo por um tempo: `next` foi de 16.2.9 pra 16.3.4 e o **CLI do `prisma` foi rebaixado de 7.8.0 pra 6.19.3**, ficando em major diferente do `@prisma/client` 7.8.0. Isso derrubou o Turbopack com o dev server rodando e passou a quebrar o `next build` no type check (`prisma.config.ts`: o `defineConfig` do Prisma 6 exige `url: string`, o do 7 aceita `undefined`). Nada disso veio das mudanças deste commit — `package.json` estava intocado por elas. Revertido pro que está no HEAD, `npm install` e `prisma generate` refeitos, build voltando a passar.
- Validado: typecheck e lint limpos (o único achado do `AiAssistant.tsx` é o `set-state-in-effect` pré-existente), `next build` compilando. **Teste real de ponta a ponta com a Groq (25 asserções verdes)**: criação com responsável resolvido pelo nome, criação em conjunto, comentário e item de checklist, alteração ficando pendente sem mexer na tarefa e sem o assistente afirmar que já fez, alvo ambíguo virando pergunta, recusa de apagar e ausência de ação quando só relataram um problema. **Mais 19 asserções determinísticas no caminho de escrita, sem depender da IA** — confirmar, cancelar, dupla confirmação, ação de outra pessoa, sem sessão, expirada, tarefa em conjunto e sobrevivência ao F5. Ressalva honesta: **não houve uma rodada única com tudo verde** — a bateria com IA estourou a cota diária da Groq no meio da segunda execução, então a cobertura das ferramentas veio de duas rodadas complementares (a primeira validou o comportamento da IA, a segunda o caminho de escrita, que antes falhava por `.next` sujo de alternar `build` e `dev`).
- Squad sintético removido ao final; conferido no banco: 5 squads reais, 257 tarefas, 0 `AssistantAction` e 0 tarefa com `source: "assistente"`.

## 2026-09-08 (6 ferramentas novas no assistente de IA, incluindo a primeira que escreve)

- Usuário perguntou o que valeria a pena dar de ferramenta pro assistente. Levantado o que a plataforma guarda contra o que ele conseguia ler: **69 Meet Recaps com transcrição completa, 84 reuniões já realizadas, 449 linhas de histórico de tarefa e a carga do squad inteiro estavam invisíveis** pras 7 ferramentas que existiam. Escolhidas 5 de leitura + 1 de escrita.
- **`search_meet_recaps`** — o maior ganho: busca dentro do conteúdo das atas ("o que ficou combinado com a Allebras sobre rebalanceamento?"), coisa que não tinha resposta em lugar nenhum do app. Duas medições mudaram o desenho: as transcrições somam ~200 mil caracteres (~50 mil tokens), então devolve **trecho de ~500 caracteres em volta do que casou**, nunca o corpo inteiro; e **só 1 dos 69 recaps tem o campo `client` preenchido** (é a IA que preenche, e a extração está pausada desde 2026-07-21), então filtrar por cliente procura o nome **no assunto e no corpo** em vez de confiar na coluna — senão a ferramenta acharia 1 recap de 69.
- **`get_team_workload`** — abertas, atrasadas, alta prioridade e concluídas nos últimos 7 dias por pessoa, separando o que ela é dona do que só participa (tarefa em conjunto: a cobrança é do dono). **`get_task_detail`** — histórico, comentários, checklist, links e partes de uma tarefa, buscada por título porque o assistente nunca tem o id. **`get_meetings_history`** — reuniões passadas, que o `get_upcoming_meetings` não cobria. **`get_squad_stats`** — criadas/concluídas no período com comparação contra o período anterior.
- **`propor_tarefa` é a primeira ferramenta que escreve — e mesmo assim não cria tarefa.** Cria uma `ExternalSuggestion` com `source: "assistente"`, pendente em /sugestoes-ia, exatamente como as do n8n e dos Meet Recaps: a regra da plataforma continua de pé, a IA nunca age sozinha e quem transforma em tarefa é uma pessoa clicando "Adicionar". Passa pela mesma `findDuplicateNote` do webhook do n8n, registra no `sourceRef` a pedido de quem nasceu, e loga em `PlatformLog`. Na tela, sugestão vinda do assistente aparece com ícone de Sparkles em vez do de workflow.
- Pra isso o `runTool` passou a receber um `ToolContext` (quem está perguntando) — as de leitura seguem escopadas só por squad. No prompt entraram o limite explícito da ferramenta de escrita ("só quando pedirem de forma clara; nunca por iniciativa própria só porque encontrou um problema — relate e pergunte") e um guia de qual ferramenta serve pra qual tipo de pergunta.
- Validado: typecheck e lint limpos, `next build` compilando, 13 ferramentas registradas. **Teste real contra a Groq, 17 asserções**, com dev server, login por credenciais e um cenário sintético (2 atas — uma de cliente e uma interna —, tarefa travada com comentários/histórico/checklist, reuniões passadas e futura, carga desequilibrada entre duas pessoas): achou o combinado na ata certa sem confundir com a interna, disse "não encontrei" para assunto que não existe em ata nenhuma, apontou quem está sobrecarregado com número, explicou por que a tarefa travou citando o comentário, contou as reuniões passadas **sem incluir a futura**, trouxe os números do período, **não propôs nada** quando só relataram um problema, e ao pedido explícito criou a sugestão pendente sem criar Task nenhuma.
- Squads sintéticos removidos ao final — inclusive **dois órfãos de tentativas de seed que falharam no meio** (o `CalendarEvent` exige `googleId`, que meu script não passava): eram 10 tarefas de teste que ficaram no banco entre uma rodada e outra. Conferido depois: 5 squads reais, 257 tarefas, nenhum squad `__TESTE`, e 0 sugestões com `source: "assistente"`.

## 2026-09-08 (assistente de IA errando a data: "hoje é 31 de agosto" quando era 9 de setembro)

- Usuário reportou o assistente alucinando e errando datas — perguntou as tarefas com prazo pra hoje e ele respondeu que hoje era 31 de agosto. **A causa não era o modelo não saber a data: era a memória da conversa.** O assistente recarrega as últimas 30 mensagens da pessoa como contexto, sem nenhuma marca de quando foram ditas — então uma resposta dele mesmo de 31/08 ("Hoje é 31 de agosto de 2026") voltava pro prompt parecendo atual, e o modelo repetia.
- **Reproduzido antes de corrigir**, com conversa de 31/08 plantada na memória e o código do HEAD: **3 de 3 rodadas** responderam "Hoje é 31 de agosto de 2026". Sem essa memória antiga, o mesmo código acertava 3 de 3 — que é justamente por que o bug parecia aleatório. Com a correção e a mesma memória envenenada: 3 de 3 corretas.
- **Correção em três frentes**, porque só carimbar a data não bastava:
  - **Data de hoje no prompt**, em mensagem de sistema própria montada a cada request e posicionada logo antes da pergunta (mais perto do fim do contexto, onde o modelo presta mais atenção, e longe das datas do histórico). Traz hoje/ontem/amanhã/+7 dias já calculados **em horário de Brasília**, mais a instrução explícita de nunca deduzir o dia por conta própria nem por datas de mensagens antigas.
  - **Histórico carimbado com a data de cada mensagem** (`[2026-08-31] ...`), pro modelo distinguir o que é de agora do que é de semanas atrás.
  - **`dueRelative` novo em `search_tasks`** (`atrasadas`/`hoje`/`amanha`/`esta_semana`/`proximos_7_dias`/`sem_prazo`), resolvido **no servidor** com o dia de Brasília. Antes o modelo tinha que converter "hoje" pra `YYYY-MM-DD` sozinho e preencher `dueBefore`/`dueAfter` — com a data errada, a lista vinha errada em silêncio, sem nenhum sinal de que era chute.
- **Três bugs de data achados no caminho, todos corrigidos:**
  - `isTaskOverdue` comparava com a meia-noite **local do servidor**. Na Vercel (UTC) o dia virava às 21h de Brasília, então toda tarefa que vencia "hoje" aparecia como atrasada nas últimas 3 horas do dia — no app inteiro, não só no assistente. Agora compara com `brtNow().today`, com os dois lados na mesma convenção (meia-noite UTC do dia).
  - `fmtDate` era usado tanto pra campo de dia (`dueDate`, prazo de tratativa) quanto pra instante (`createdAt`, início de reunião). Separado em `fmtDay` (lê em UTC, que é como o dia fica gravado) e `fmtMoment` (lê em America/Sao_Paulo) — uma reunião das 09:00 chegava ao modelo como "12:00Z" e ele reportava o horário errado.
  - `search_tasks` tinha **duas chaves `OR` irmãs** no mesmo objeto de `where` (uma do nome do responsável, outra do texto livre) — em JS a segunda sobrescreve a primeira, então "tarefa da Tainara sobre balancete" ignorava a Tainara calado. Passaram a compor em `AND[]`. Regressão minha do commit de tarefas em conjunto, de hoje mais cedo.
- Temperatura de 0.3 → **0**: é um assistente que relata dado real, variação aqui só aumenta a chance de "arredondar" um número ou um nome. Somadas três regras anti-alucinação ao prompt: só afirmar data vinda do sistema ou de ferramenta; responder só com o que a ferramenta devolveu (lista vazia é "não há", não é hora de completar com exemplo nem repetir resposta anterior); e não inventar nome de cliente, pessoa, título ou número.
- Validado: typecheck e lint limpos. **Teste real de ponta a ponta contra a Groq de verdade, 11 asserções**, com dev server, login por credenciais e um cenário de prazos conhecidos (2 tarefas pra hoje, 1 pra amanhã, 1 daqui 9 dias, 1 atrasada, 1 sem prazo, 1 concluída hoje) — **rodado com a memória antiga de 31/08 plantada**, que é a condição em que o bug aparecia: acertou a data, trouxe exatamente as 2 de hoje sem incluir amanhã/futuras/atrasada/sem prazo, listou como atrasada só a que era, e disse que não havia nada da "Petrobras" em vez de inventar. Squad/usuário/tarefas sintéticos removidos ao final; conferido no banco: 257 tarefas reais intactas.
- **Não mexido:** o teto de 30 mensagens de memória (`ASSISTANT_HISTORY_LIMIT`) continua igual — com o histórico carimbado e a data no prompt, o contexto antigo deixou de atrapalhar, então não há motivo pra encurtar a memória.

## 2026-09-08 (tarefas em conjunto: vários responsáveis, cada um com a sua parte)

- Usuário pediu poder botar mais de um responsável na tarefa ("Felipe, Tainara e Gustavo", "Felipe e cliente", "cliente e Gustavo"), com a tarefa aparecendo pra todos e marcada de forma especial. Mapeado antes de escrever código: **164 usos de `assignee` em 30 arquivos** (filtros de Tarefas/Kanban/Semana/Dashboard, cards, sino, Slack, e-mail, prazo, digest, briefing, assistente, recorrência, busca). Três decisões foram do usuário: cada um marca a **sua parte** (a tarefa só fecha quando todas estiverem marcadas), tem **um dono + participantes**, e **aviso só pro dono**.
- **Modelo (`db push`, sem migration — banco compartilhado dev+prod):** novo `TaskAssignee` (taskId, userId ou isClient, role principal/participant, `part`, `done`/`doneAt`/`doneById`, sortOrder). A escolha central: **tarefa de uma pessoa só continua exatamente como era** (`Task.assigneeId`, ou assigneeId null + deliverTo "o2" pro cliente) e **não ganha nenhuma linha** — `assignees.length > 0` é o que significa "em conjunto". Isso deixou intactos os ~160 pontos que leem `assigneeId`, em vez de migrar as 257 tarefas existentes e mexer em tudo de uma vez. `Task.assigneeId` segue sendo o dono, espelhado da linha `role="principal"`.
- **`src/lib/task-assignees.ts` é a fonte única** de "quem é responsável": `taskResponsibles()` resolve as duas formas num formato só (é o que a UI usa pra avatar, filtro e checklist — ninguém lê `assigneeId` direto pra isso), `isResponsible()` é o que faz a tarefa aparecer pra participante, e `normalizeAssignees()` concentra as regras (o primeiro é o dono; o dono precisa ser do squad, porque é ele quem recebe aviso; sem repetido; cliente no máximo uma vez).
- **Status e partes andam juntos**, senão dava pra ter tarefa "concluída" com parte pendente: marcar a última parte fecha a tarefa; desmarcar uma parte de tarefa fechada reabre em "Em andamento"; concluir direto (arrastar no Kanban) marca todas as partes; reabrir desmarca todas. `Task.status` continua sendo o campo simples de sempre — nada no Kanban, atraso ou digest precisou virar valor derivado.
- **UI:** `AssigneePicker` novo, usado nos **três fluxos** (nova tarefa, edição da tarefa e aceitação de sugestão da IA), com badge "Dono" no primeiro, campo de "parte" por pessoa e o cliente na lista quando há cliente preenchido. No painel da tarefa, seção "Partes (2/3)" com barra de progresso e checkbox por responsável — a do cliente é marcada por alguém do squad, que fica registrado em `doneById`. No card, pilha de avatares (com o dono na frente, quem já concluiu esmaecido) e selo verde "2/3" marcando que é em conjunto.
- **Filtros normalizados:** Tarefas, Kanban e Semana passaram a casar por dono **ou** participante. No Kanban o filtro virou `some` sobre os selecionados de propósito — filtrar por Felipe e Tainara ao mesmo tempo não pode fazer a mesma tarefa aparecer duas vezes. A recorrência herda os responsáveis e as partes (as partes nascem desmarcadas: é uma rodada nova). O assistente de IA acha por nome tanto quem é dono quanto quem só participa.
- **Avisos só pro dono, como pedido:** participante não recebe sino/Slack/e-mail ao ser incluído, e a cobrança de prazo, a carga do Dashboard e o digest semanal seguem contando pro dono. O único aviso novo é **pro dono**, quando alguém conclui uma parte. Se algum dia isso incomodar, o ponto de mudança é um só (`assigneeId` continua sendo "o dono").
- **Dois bugs meus pegos pelo teste, antes de subir:** (1) o PATCH montava a resposta **antes** de gravar os responsáveis, devolvendo a lista velha — a tela mostraria o estado anterior até dar refresh; (2) fechar a tarefa marcando a **última parte** não gerava a próxima ocorrência da série — a rotina recorrente morreria calada, exatamente o que o cron de 2026-08-31 existe pra evitar (o spawn só estava no caminho "arrastar pra Concluída"). Os dois corrigidos e cobertos por asserção.
- Validado: typecheck limpo, `next build` compilando, lint sem erro novo (8 achados antes e 8 depois — todos `set-state-in-effect` pré-existentes; os 3 arquivos novos não têm nenhum). **Teste real de ponta a ponta, 40 asserções**, com dev server, dois logins de verdade (dono e participante) e squad/usuários sintéticos removidos ao final: criação com 3 responsáveis, participante enxergando a tarefa pela API, participante não recebendo aviso, partes marcadas uma a uma fechando só na última, parte do cliente marcada pelo squad, reabertura, concluir/reabrir pelo status, edição preservando partes já concluídas, troca de dono, cliente recusado como dono (400), responsável de fora do squad recusado (400), tarefa solo e tarefa do cliente seguindo sem nenhuma linha, ida e volta entre solo e conjunto, recorrência herdando responsáveis, histórico e 401 sem sessão. As 6 telas também foram carregadas logado como participante. Banco real conferido depois da limpeza: 257 tarefas intactas e 0 linhas de `TaskAssignee` (nada real usa ainda).

## 2026-09-08 (não dava pra mudar o responsável para o cliente depois da tarefa criada)

- Usuário reportou que, depois de adicionar e salvar uma tarefa, não dá pra trocar o responsável para o cliente. Confirmado: "responsável = Cliente" é `assigneeId: null` + `deliverTo: "o2"`, e o `deliverTo` **só existia na criação**. Faltava nos dois lados ao mesmo tempo, então não tinha nem como contornar pela API.
- **API:** `PATCH /api/tasks/[id]` nem lia `body.deliverTo` — respondia 200 e ignorava o campo calado. Agora aceita e valida (`""` = interna, `"client"`, `"o2"`), recusando outro valor com 400 em vez de virar null silenciosamente (mesmo critério já usado pra recorrência e horário).
- **Tela:** o formulário de edição do `TaskDetailPanel` não tinha o campo Entrega nem a opção "Cliente" no Responsável — só "Sem responsável" e as pessoas do squad, enquanto o `NewTaskModal` já oferecia as duas coisas desde 2026-08-17. Adicionados os dois, com as **mesmas regras de sincronia** do modal de criação: escolher "Cliente" já marca a Entrega como "Cliente entrega para a O2"; apagar o nome do cliente ou mudar a Entrega tira "Cliente" do Responsável. A tarefa que já é do cliente agora abre com "Cliente" selecionado, não com "Sem responsável".
- **Isso já tinha produzido inconsistência real:** sem a regra de sincronia na edição, dava pra salvar uma tarefa com responsável do squad **e** marcada como "cliente entrega para a O2" — ela some do filtro "Cliente" no Kanban/Tarefas (que exige `!assignee`) mas continua aparecendo no calendário da reunião como pendência do cliente (que só olha `deliverTo`). Duas tarefas da Allebras estão nesse estado ("Revisar custos e estoque" e "Trabalhar na planilha de importação", ambas com o Gustavo) — perguntado, o usuário optou por **deixar as duas como estão** — não é pra "corrigir" isso depois; agora dá pra arrumar pela própria tela se um dia quiser.
- Histórico da tarefa passou a registrar mudança de entrega (`deliver_to`, "Entrega: interna → Cliente entrega para a O2"). De quebra, `recurrence` e `deliver_to` entraram no mapa de verbos do painel — `recurrence` já gravava desde 2026-08-31 mas aparecia como o genérico "atualizou a tarefa".
- Validado: typecheck limpo, lint sem erro novo (o `set-state-in-effect` do `TaskDetailPanel` é pré-existente, conferido contra o HEAD). **Teste real de ponta a ponta, 15 asserções**, com dev server e login por credenciais de verdade, em squad/usuários/tarefa sintéticos removidos ao final — atribuir ao cliente depois de criada, voltar pra uma pessoa limpando a entrega, "O2 entrega para o cliente" sem mexer no responsável, valor inválido recusado com 400 sem corromper o que estava salvo, as 3 linhas no histórico e 401 sem sessão. O mesmo teste rodado contra o código do HEAD mostra o bug: pedindo `deliverTo: "o2"` numa tarefa que estava em `"client"`, a API respondia 200 e o banco continuava `"client"`.

## 2026-09-08 (aba "Excluídos" das sugestões da IA estava sempre vazia)

- Usuário reportou, testando a plataforma na reunião, que descartar uma sugestão da IA não deixava ela guardada na lista de tarefas excluídas. Confirmado no banco: o descarte **sempre gravou** — 155 `ExternalSuggestion` com status `rejected` no squad da O2. Quem escondia era a listagem.
- **Causa:** `GET /api/suggestions/external` filtrava `status: { not: "rejected" }`. A página `/sugestoes-ia` monta as três abas a partir dessa mesma resposta, então a aba "Excluídos" nunca recebia nada. Dava pra ver a sugestão migrar pra aba no instante do clique (a tela troca o status localmente, sem refetch) e ela sumia no primeiro F5 — exatamente o "não fica armazenada" que o usuário descreveu. O caminho do Meet Recap não tinha o problema (`/api/recaps` só exclui `superseded`), mas a extração por IA dos recaps está pausada desde 2026-07-21, então **na prática toda sugestão hoje vem do n8n** — ou seja, a aba estava 100% vazia. Trocado por uma lista explícita dos status que a tela exibe (`pending`/`duplicate`/`rejected`); `accepted`/`edited` seguem de fora porque já viraram tarefa no Kanban.
- **Segundo caminho, mesmo sintoma:** deletar no Kanban uma tarefa que nasceu de sugestão. `DELETE /api/tasks/[id]` apagava só a `Task`; a sugestão ficava `accepted`/`edited` com `taskId` zerado pelo `SetNull` do banco — fora do Kanban e fora das três abas, sumindo de vez. Agora a exclusão devolve a sugestão pro status `rejected` e desvincula a tarefa, então ela cai em "Excluídos" e dá pra restaurar. O DELETE passou a confirmar antes que a tarefa é do squad da sessão (`RecapSuggestion` não tem `squadId` próprio e é achada por `taskId`, então sem essa checagem daria pra mexer na sugestão de outro squad).
- 4 sugestões antigas do squad da O2 já estavam nesse estado órfão ("Envio fluxo de caixa Cap Table", "Revisar Balancetes", "Definição base D2M", "Enviar Relatório Despesas", de 13 a 21/08). Backfill aplicado no banco: as quatro passaram pra `rejected` e voltaram a aparecer em "Excluídos", de onde dá pra restaurar. Zero órfãs restantes (external e recap).
- **Lacuna conhecida, não mexida:** `findDuplicateNote` só compara com sugestões `pending` e tarefas abertas, então uma sugestão descartada pode voltar como pendente na leva seguinte do n8n sem ser marcada como duplicada. Medido no banco: 0 de 111 pendentes hoje são reaparição de algo descartado — é lacuna teórica, não está acontecendo.
- Validado: typecheck e lint limpos; consulta nova simulada direto contra o banco de produção, devolvendo 111 pendentes / 155 excluídos pro squad da O2 (antes a aba vinha com 0).

## 2026-08-31 (recorrência de tarefa com dia da semana e horário — rotina tipo "importação toda terça e sexta às 9h")

- Usuário pediu recorrência pra rotinas de importação ("toda terça e sexta"), com horário e aviso na hora. A recorrência que existia era `Task.recurrence` = weekly/biweekly/monthly e tinha **três limitações**: (1) não dava pra expressar dois dias na semana — só somava 7/14/30 dias em cima da última data; (2) não tinha horário nenhum, então não havia o que lembrar; (3) a próxima ocorrência só nascia ao marcar "concluída" — quem esquecia de fazer a da terça ficava sem a da sexta e a rotina morria calada. Confirmado no banco: **nenhuma** tarefa usava recorrência (o recurso estava inutilizável na prática).
- **Schema (`db push`, sem migration — banco compartilhado dev+prod):** `Task.dueTime` ("HH:MM", horário de Brasília), `Task.recurrenceWeekdays Int[]` (0=dom..6=sáb, ex: `[2,5]`) e `Task.recurrenceSpawned Boolean`. O horário ficou num campo separado de propósito: `dueDate` é só o dia (meia-noite UTC, ver `dueDateOnly`) e mudar essa semântica mexeria em kanban, semana, atraso, digest e briefing de uma vez.
- **`src/lib/recurrence.ts`** (sem Prisma, então UI e API usam as mesmas funções): `nextOccurrence`/`firstOccurrence` — em "weekdays" a próxima data é o primeiro dia da lista depois da âncora —, validação (`isValidRecurrence`, `isValidTime`, `normalizeWeekdays`) e `describeRecurrence` ("Terça e sexta às 09:00", usado na tela e no texto da notificação). Toda conta de data usa getters UTC: com getters locais, servidor em fuso negativo leria o dia anterior e o dia da semana sairia errado.
- **A série não depende mais de ninguém concluir:** novo job `task-reminders` (de 5 em 5 min) materializa a ocorrência seguinte assim que o dia da atual passa. `recurrenceSpawned` é o que garante idempotência — o cron roda a cada 5 min e a conclusão também dispara isso; de bônus fecha um bug antigo em que concluir → reabrir → concluir criava duas ocorrências. A âncora é a data mais recente entre o prazo da ocorrência e hoje: concluir adiantado mantém o ritmo (terça → sexta) e tarefa esquecida há meses não gera um monte de ocorrência no passado.
- **Aviso no horário** (`checkTaskDueTimes`): sino + DM no Slack (tipo novo `taskDueTime` em Configurações → Slack, ligado por padrão) pra quem é responsável — ou pra quem criou, se a tarefa for do cliente. A condição é "já passou da hora", não "está na janela de 5 min", porque o agendador do GitHub Actions atrasa com frequência e uma janela fixa engoliria o aviso do dia; o dedup por dia garante um aviso só. Horário resolvido em `America/Sao_Paulo` via `brtNow()` em `utils.ts` — o servidor da Vercel roda em UTC, comparar "09:00" com `new Date()` erraria 3 horas e "hoje" viraria o dia seguinte depois das 21h.
- **UI:** campo Horário ao lado do Prazo (criação e edição), opção "Em dias da semana" na Recorrência revelando um seletor de dias (`WeekdayPicker`), resumo em texto do que foi configurado, horário no card e no detalhe da tarefa, e histórico registrando mudança de recorrência/horário. O job também entrou no `instrumentation.ts` (node-cron do dev) e no `.github/workflows/cron.yml` — o slot de 5 min agora chama dois jobs em sequência, em vez de uma segunda entrada `*/5` (que não daria pra distinguir em `github.event.schedule` e dobraria os minutos do Actions).
- **Pontualidade do aviso (decisão do usuário):** medidas as 20 execuções anteriores do `schedule` do GitHub Actions via API — intervalos reais de **7 min a 428 min**, incluindo um buraco de 7 horas de madrugada. Pra sync de Gmail isso é só atraso, mas pra "avise às 9h" quebra a utilidade. Perguntado, o usuário escolheu **agendador externo gratuito** (cron-job.org) chamando só o `task-reminders` a cada 5 min; o resto dos jobs fica no GitHub Actions. Passo a passo em `docs/agendador-externo.md`. Isso **revisa** a decisão de 2026-07-10 de manter tudo no GitHub Actions — não por preferência, mas porque surgiu um recurso em que pontualidade é o ponto.
- Pra isso, `/api/cron/[job]` passou a aceitar o secret também como `?key=<secret>`, além do header `Authorization: Bearer` (vários serviços de ping gratuitos só sabem chamar uma URL, sem header customizado). Menos seguro — secret aparece em log/URL —, então documentado como fallback, com o header seguindo preferível. As 4 combinações foram testadas: header correto → 200, `?key=` correto → 200, `?key=` errado → 401, sem nada → 401.
- Validado: typecheck limpo, lint sem erro novo (o `set-state-in-effect` do `TaskDetailPanel` é pré-existente, conferido contra o HEAD). **Teste real de ponta a ponta, 25 asserções, em squad/usuário sintéticos removidos ao final** — cron gerando a ocorrência de terça→sexta com horário/dias/cliente/responsável herdados, segunda passada sem duplicar nada, notificação disparando só depois da hora (e uma vez só), login real via credenciais criando tarefa pela API com dias+horário, série ancorando na primeira data válida quando não tem prazo, conclusão gerando a próxima, reabrir+concluir sem duplicar, e recorrência/horário inválidos recusados com 400.
...
- **Novo model `PlatformOwner`** (`{userId}`, fora de `SCOPED_MODELS` de propósito) — dimensão totalmente separada do `role` de squad (admin/membro). Só gerenciável manualmente (nunca por UI, nunca auto-concedível por um admin de squad qualquer). Concedido pro Gustavo e pro Felipe.
- **Sessão ganhou `isOwner`**, recomputado a cada requisição (mesmo padrão já usado pra `squadId`/`role`) — conceder ou revogar acesso reflete na hora, sem precisar deslogar.
- **Nova rota `/owner`** (fora do grupo `(app)`, não herda o Sidebar de squad) — cabeçalho + tabela comparativa por squad: usuários (admin/membro, pendente onboarding), tarefas (total + última atividade), clientes (total + ativos), Meet Recaps (processados/total), taxa de aceite da IA, automações (ativas + sucesso 30d), mensagens com o assistente. Reaproveita 100% o vocabulário visual já existente (stat tiles de `/automacoes`, tabela de `ClientsTable`) — nenhuma lib de gráfico nova.
- **Escopo deliberado**: só métricas agregadas (contagens/taxas) — não um modo de abrir e navegar no dado de negócio real (tarefa, cliente) de outro squad. `GET /api/owner/metrics` usa `prisma` cru (nunca `forSquad`), confinado a `src/app/owner/**` — o resto do código de squad continua 100% obrigado a passar pelo isolamento central.
- **Validado com 13 checagens reais**: admin comum de squad tem `isOwner` falso e é barrado (403/redirect) de `/owner` e da rota de métricas; conceder `PlatformOwner` reflete na MESMA sessão sem re-login; owner acessa tudo; as métricas incluem os 3 squads reais + um squad de teste recém-criado com os números certos; soma de usuários reportada bate exatamente com a contagem direta no banco (9 = 9); revogar o acesso também reflete na hora. Squad de teste e concessão temporária removidos depois — confirmado que só Gustavo e Felipe continuam com `PlatformOwner`.
